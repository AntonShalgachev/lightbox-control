...
public:
	...
	static void SendMessage(uint8_t msg, uint8_t len, const uint8_t* data);
	static void SendByte(uint8_t b);

private:
	uint8_t _buffer[UINT8_MAX + 2]; // MSG_ID (1 byte), DATA_LEN (1 byte), DATA (DETA_LEN bytes)
	State state;
	uint16_t _bytesRead;
	MsgHandler _handler;
	...
	static const uint8_t RPRT_FAIL = 0x00;
	static const uint8_t RPRT_OK = 0x01;
	static const uint8_t RPRT_INVALID_MSG_ID = 0x02;
	static const uint8_t RPRT_MALFORMED_MSG = 0x03;
};
