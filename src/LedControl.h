...
public:
	...
	void SetRGBSingleLed(uint8_t led, const Color& color, uint8_t brightness = 0xff);
	void SetRGBLed(uint8_t led, const Color& color, uint8_t brightness = 0xff);
...
private:
	...
	uint8_t _brightnessWhite;
	uint8_t _brightnessRGB;

	Color _colorData[NUM_LEDS];
	uint8_t _brightnessData[NUM_LEDS];
	...
	static const uint16_t LED_BRIGHTNESS_MAX_BITS = 11; // 1024
